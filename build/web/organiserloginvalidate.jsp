...
<body>
      <sql:setDataSource 
		...
		<sql:query dataSource="${usr}" var="result1" >
				select * from organiser where o_username=? 
				<sql:param value="${param.o_username}"/>
		</sql:query>
		...
		<sql:query dataSource="${usr}" var="result2" >
			select * from organiser where o_username=? and o_password=?
			<sql:param value="${param.o_username}"/>
			<sql:param value="${param.o_password}"/>
		</sql:query>
		...
		<c:set var="flag2" scope="application" value="1"/>
		    ...
		    <c:set var="flag2" scope="application" value="1"/>
				...
				<h1>
					Welcome !!  ... <c:out value="${row2.o_username}"/>
                                        email : <c:out value="${row2.o_email}"/>
